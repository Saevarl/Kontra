...
def generate_rules_yaml(profile: DatasetProfile) -> str:
    ...
    lines = [
        f"# Auto-generated contract",
        f"# Source: {profile.source_uri}",
        f"# Rows analyzed: {profile.row_count:,}",
        f"# Generated by Kontra v{profile.engine_version}",
        "",
        "name: suggested_contract",
        f"description: Auto-generated from profile",
        "",
    ...
    for rule in rules:
        ...
        severity = rule.get("severity")  # None if not specified (defaults to blocking)
        ...
        # Add severity if not the default (blocking)
    ...
    if col.dtype in ("datetime", "timestamp") or col.temporal is not None:
        ...
        if any(hint in name_lower for hint in freshness_hints):
            rules.append({
                ...
                "severity": "warning",  # Default to warning - historical data would fail blocking
                "_comment": f"{col.name}: timestamp column, adjust max_age and severity as needed",
            })
