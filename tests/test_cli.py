...
class TestScout:
    ...
    def test_scout_suggest_rules(self, sample_parquet):
        """Scout --suggest-rules generates contract YAML."""
        result = runner.invoke(app, [
            "scout", str(sample_parquet),
            "--suggest-rules"
        ])
    ...
    def test_scout_columns_filter(self, sample_parquet):
        """Scout with --columns filter."""
        result = runner.invoke(app, [
            "scout", str(sample_parquet),
            "--columns", "id,name"
        ])
    ...
    def test_scout_sample(self, sample_parquet):
        """Scout with --sample."""
        result = runner.invoke(app, [
            "scout", str(sample_parquet),
            "--sample", "3"
    ...
    def test_scout_include_patterns(self, sample_parquet):
        """Scout with --include-patterns."""
        result = runner.invoke(app, [
            "scout", str(sample_parquet),
            "--include-patterns"
        ])
    ...
    def test_scout_file_not_found(self):
        """Scout returns non-zero exit code for missing file."""
        result = runner.invoke(app, ["scout", "nonexistent.parquet"])
        assert result.exit_code != 0  # Could be 2 (config error) or 3 (runtime error)
