...
- Known issues: NaN vs NULL pitfalls (unique semantic mismatch resolved in v0.5.2)
...
**Built-in Rules** (18):
- Column: `not_null`, `unique`, `allowed_values`, `disallowed_values`, `range`, `regex`, `dtype`, `length`, `contains`, `starts_with`, `ends_with`
...
10. **Separate counting vs sampling predicates**: Some rules (like `unique`) have different semantics for counting violations vs identifying failing rows. Use `compile_predicate()` for vectorized counting, and `sample_predicate()` when sampling needs different logic. The sampling code checks `sample_predicate()` first.

11. **SQL vs Polars consistency**: When adding SQL pushdown, ensure Polars uses the same semantics. For `unique`, SQL uses `COUNT(*) - COUNT(DISTINCT col)` (extra rows), so Polars must too. Don't let execution tiers return different counts.

12. **Test config vs running containers**: SQL Server tests use port from `conftest.py`. If container is running on different port, tests fail with connection errors. Keep `docker-compose.yml` and `conftest.py` in sync.

13. **LIKE vs REGEX for string rules**: Use LIKE patterns (`%substring%`) instead of regex for `contains`, `starts_with`, `ends_with`. LIKE is 3-6x faster on databases. Only use regex for `regex` rule where pattern matching is required.
...
**Never put version numbers in commit messages.** Commit messages describe *what changed*, not the version.

```bash
# GOOD
git commit -m "Add 5 string validation rules, enable unique SQL pushdown"

# BAD
git commit -m "Release 0.5.2: Add string rules"
git commit -m "Bump version to 0.5.2"
```

Version numbers belong in **git tags**, not commit messages. The tag is the source of truth.

### Release Process

```bash
# 1. Commit changes with descriptive message (no version)
git add .
git commit -m "Add feature X, fix bug Y"

# 2. Update version in pyproject.toml AND src/kontra/version.py
# 3. Amend previous commit OR create minimal commit
git add pyproject.toml src/kontra/version.py
...
source .env && twine upload dist/* --username __token__ --password "$TWINE_PASSWORD"
```

### Semantic Versioning

- **PATCH** (0.5.1 → 0.5.2): Bug fixes, small additions
